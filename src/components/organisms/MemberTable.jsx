...
const FamilyIndicator = ({ member }) => {
  ...
  useEffect(() => {
    const loadFamilyInfo = async () => {
      try {
        const familyMembers = await memberService.getFamilyMembers(member.Id);
        setFamilySize(familyMembers.length);
        if (familyMembers.length > 0) {
          setFamilyName(familyMembers[0].lastName);
        }
      } catch (error) {
